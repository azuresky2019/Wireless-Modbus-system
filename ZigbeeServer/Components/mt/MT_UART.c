...
#if defined ( JUMP_MACHINE) 
  #define JUMP_STATE_1   0x06
  #define JUMP_STATE_2   0x07
  #define JUMP_STATE_3   0x08
#endif
/***************************************************************************************************
 *                                         GLOBAL VARIABLES
 ***************************************************************************************************/
...
#if defined ( RFID_SUPPLY)
{
  ...
  {
    HalUARTRead( port, &head, 1);
    HalUARTRead( port, &cmd, 1);
    
    if( (cmd == MODBUS_SINGLE_READ ) || ( cmd == MODBUS_SINGLE_WRITE))
    {
      pMsg = osal_mem_alloc( 8);
      pMsg[0] = head;
      pMsg[1] = cmd;
      HalUARTRead( port, &pMsg[2], 6);
      modbus_uart_data_process( pMsg, 8);
      osal_mem_free(pMsg);
    }
    else if( cmd == MODBUS_MULTI_WRITE)
    {
      uint8 modbusId_H,modbusId_L,len_H,len_L;
      HalUARTRead( port, &modbusId_H, 1);
      HalUARTRead( port, &modbusId_L, 1);
      HalUARTRead( port, &len_H, 1);
      HalUARTRead( port, &len_L, 1);
      pMsg = osal_mem_alloc(len_L*2+9);
      pMsg[0] = head;
      pMsg[1] = cmd;
      pMsg[2] = modbusId_H;
      pMsg[3] = modbusId_L;
      pMsg[4] = len_H;
      pMsg[5] = len_L;
      HalUARTRead( port, &pMsg[6], (len_L*2+3));
      modbus_uart_data_process( pMsg, len_L*2+9);
      osal_mem_free(pMsg);
    }
  }
}
#else
{
  ...
  while (Hal_UART_RxBufLen(port))
  {
    ...
    switch (state)
    {
      ...
      case JUMP_STATE_3:
        TxBuffer.buf[TxBuffer.size++] = ch;
        if( TxBuffer.size >= (TxBuffer.buf[2] + 5))
        {
          afAddrType_t timeAddr;
          uint8 TransID;
          
          TransID = 0;
          timeAddr.addrMode = afAddr16Bit;
          ...
          AF_DataRequest( &timeAddr, &GenericApp_epDesc,
                                       ...
                                       AF_DEFAULT_RADIUS );
          TxBuffer.size = 0;
    //      osal_set_event( GenericApp_TaskID, MSG_PROGRESS_EVT);
          state = SOP_STATE;
        }
        break;
#endif

...
#if defined ( JUMP_MACHINE) || defined ( CC2530_TIME_SYNC_ROUTER) || defined ( CC2531_TIME_SYNC_COORD)
...
#if defined ( CC2530_TIME_SYNC_ROUTER) || defined ( CC2531_TIME_SYNC_COORD)
void MT_UtilGetMicroTime( void);
void MT_UtilSetMicroTime( uint8 *pBuf);
#endif
#if defined ( GATE_PROTOCAL)
void MT_UtilSendGateProtocalCmd( uint8 *pBuf);
void MT_UtilSendCmdWithAddr( uint8 *pBuf);
#if defined ( JUMP_MACHINE)
void MT_UtilConfirmRfidRsp( uint8 *pBuf);
#endif
#endif
...
// CC253X MAC Network Processor does not have NV support
    ...
    case MT_UTIL_TIME_ALIVE:
      MT_UtilTimeAlive();
      break;
#if defined ( CC2531_TIME_SYNC_COORD) || defined ( CC2530_TIME_SYNC_ROUTER)
    case MT_UTIL_GET_MICRO_TIME:
      MT_UtilGetMicroTime();
      break;
      
    case MT_UTIL_SET_MICRO_TIME:
      MT_UtilSetMicroTime(pBuf);
      break;
      
  #if defined ( JUMP_MACHINE)
    case MT_UTIL_CONFIRM_RFID_RSP:
      MT_UtilConfirmRfidRsp(pBuf);
      break;
  #endif
#endif
      
    case MT_UTIL_READ_SD:
      MT_UtilReadSD(pBuf);
      
      
#if defined ( GATE_PROTOCAL)
    case MT_UTIL_GATE_PROTOCAL_CMD:
      MT_UtilSendGateProtocalCmd(pBuf);
      break;
      
    case MT_UTIL_SEND_CMD_WITH_ADDR:
      MT_UtilSendCmdWithAddr(pBuf);
      break;
#endif
...
void MT_UtilGetMicroTime( void)
{
  ...
  MT_BuildAndSendZToolResponse(((uint8)MT_RPC_CMD_SRSP | (uint8)MT_RPC_SYS_UTIL),
                                       MT_UTIL_GET_MICRO_TIME, sizeof(timeAlive), timeAlive);
}
...
//  uint8 cmd[5];
  ...
  tmp32 = BUILD_UINT32( pBuf[4],
                       pBuf[5],
                       pBuf[6],
                       pBuf[7]); 
  
  osal_SetSystemClock( tmp32);
  ...
  retValue =  AF_DataRequest( &destAddr, &GenericApp_epDesc,
                               TIME_SYNC_CLUSTERID,
                               sizeof( uint8),
                               &cmd,
                               &TransID,
  ...
  cmd[0] = SET_COORD_TIME;
  cmd[1] = pBuf[4];
  cmd[2] = pBuf[5];
  cmd[3] = pBuf[6];
  cmd[4] = pBuf[7];
  
  // 每一个设备收到设置时间请求，发送同步时间命令,并设置coord的时间
   retValue =  AF_DataRequest( &destAddr, &GenericApp_epDesc,
                               TIME_SYNC_CLUSTERID,
                               5*sizeof( uint8),
                               cmd,
  ...
  MT_BuildAndSendZToolResponse(((uint8)MT_RPC_CMD_SRSP | (uint8)MT_RPC_SYS_UTIL),
                                       MT_UTIL_SET_MICRO_TIME, 1, &retValue);                     
}
#endif
...
void MT_UtilSendGateProtocalCmd( uint8 *pBuf)
{
  ...
  MT_BuildAndSendZToolResponse(((uint8)MT_RPC_CMD_SRSP | (uint8)MT_RPC_SYS_UTIL),
                                       MT_UTIL_GATE_PROTOCAL_CMD, 1, &retValue);
}
...
//  uint8 retValue;
  ...
  destAddr.addr.shortAddr = BUILD_UINT16( pBuf[MT_RPC_POS_DAT0], pBuf[4]);
  
  if( destAddr.addr.shortAddr == 0xffff)
  {
    destAddr.addrMode = afAddrBroadcast;
    destAddr.endPoint = AF_BROADCAST_ENDPOINT;
  }
  
  clusterID = BUILD_UINT16( pBuf[5], pBuf[6]);
  
  len = pBuf[MT_RPC_POS_LEN] - 4;
  buf = &pBuf[7];
  
  if( (pBuf[7] == 0xff) && (pBuf[8] == 0x03 ) && (pBuf[9] == 0x02))
  {
    modbus_id = pBuf[11];
  }
  else if((pBuf[7] == 0xff) && (pBuf[10] == 0xbf))
  {
    confirmRfidName = TRUE;
  }
...
//  MT_BuildAndSendZToolResponse(((uint8)MT_RPC_CMD_SRSP | (uint8)MT_RPC_SYS_UTIL),
//                                       MT_UTIL_SEND_CMD_WITH_ADDR, 1, &retValue);
}

...
void MT_UartInit ()
{
  ...
  if(SUCCESS != osal_nv_read(ZCD_NV_BAUDRATE,0, sizeof(modbus_set_baudrate), &modbus_set_baudrate))
  {
    osal_nv_item_init(ZCD_NV_BAUDRATE, sizeof(modbus_set_baudrate), &modbus_set_baudrate);
    osal_nv_write(ZCD_NV_BAUDRATE, 0,sizeof(modbus_set_baudrate), &modbus_set_baudrate);
  }
  /* UART Configuration */
  uartConfig.configured           = TRUE;
  uartConfig.baudRate             = modbus_set_baudrate;

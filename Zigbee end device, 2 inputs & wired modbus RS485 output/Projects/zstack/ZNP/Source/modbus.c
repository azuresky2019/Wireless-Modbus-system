...
uint8 modbus_id = 254;
#else
uint8 modbus_id = 23;  // 23、24两个设备 暂时
#endif
...
static void modbus_process_msg( uint8 *data_buffer, uint8 len)
{
  ...
  afAddrType_t destAddr;
  destAddr.addrMode = afAddr16Bit;//afAddrBroadcast;
  destAddr.addr.shortAddr = 0;//0xffff;                
  destAddr.endPoint = 10;
    ...
    else if( data_buffer[1] == MODBUS_SINGLE_READ)
    {
      ...
      if( pBuf != NULL)
      {
        ...
        for( i = 0; i < num; i++)
        {
          ...
          {
            uint16 power_temp;
            power_temp = (ad_power-196)*10/10.57+90;//((ad_power-644)*64)/100+70;

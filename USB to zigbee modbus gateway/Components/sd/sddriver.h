...
extern uint8 SD_ReadMultiBlock(uint32 startblock, uint32 blocknum, uint8 *recbuf); 
...
extern void SD_GetSDStatus(void); // use in the HCSD card for calculte the erase unit size and erase timeout
extern uint8 SD_GetCardInfo(void);

/* 由CSD寄存器计算超时时间 calculate timeout from CSD register */
extern uint8 SD_CalTimeout(uint8 *csdbuf);
...
#define	CMD8_VHS			0x01 // supports 2.7V-3.6V

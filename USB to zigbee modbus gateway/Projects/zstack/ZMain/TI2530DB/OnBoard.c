...
void InitBoard( uint8 level )
{
  ...
  else  // !OB_COLD
  {
    ...
    HalKeyConfig(HAL_KEY_INTERRUPT_DISABLE, OnBoard_KeyCallback);       // LJ 避免按键定义使IO口冲突
  }
}
...
void OnBoard_KeyCallback ( uint8 keys, uint8 state )
{
  ...
  (void)state;
#if defined  ( JUMP_MACHINE_DONGLE)
  uint8 keyCmd_buf[7] = {0x12,0x34,0x56,0,0,0, 0x00};
...
#if defined  ( JUMP_MACHINE_DONGLE)
      keyCmd_buf[3] = 0;
      //MT_BuildAndSendZToolResponse( 0x67, 0x14, 12, keyCmd_buf);
